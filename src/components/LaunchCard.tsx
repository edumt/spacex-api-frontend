...
const LaunchCard: React.FC<Props> = ({ launch }) => {
  ...
  return (
    <Link
      ...
      className="m-6 mx-auto grid h-96 w-4/5 max-w-2xl grid-cols-1 grid-rows-2 overflow-hidden rounded bg-black/80 text-center transition hover:scale-105 md:h-48  md:w-full md:grid-cols-3 md:grid-rows-1"
      key={launch.id}
    >
      ...
      <div className="col-span-2 flex h-full flex-col items-center justify-center gap-3 text-white">
        ...
        <p className="text-2xl">{new Date(launch.date_utc).toLocaleString()}</p>
      </div>
    </Link>
  );
};
